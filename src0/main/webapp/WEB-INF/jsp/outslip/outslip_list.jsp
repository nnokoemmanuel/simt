			...
			<div class="kt-portlet__head-wrapper">
				<div class="kt-portlet__head-actions">
					...
					<sec:authorize access="hasRole('ROLE_MANAGE_OUTSLIP')">
						<a href="#" onClick="openNav('Out Slip Generation', '', '40%');doGet('${pageContext.request.contextPath}/outSlip/create','contenu')"  class="btn btn-brand btn-elevate btn-icon-sm">
	                       <i class="la la-plus"></i>
	                         <fmt:message key="app.new.out.slip" />
	                    </a>
                    </sec:authorize>
				</div>
			</div>
...
<div class="kt-container  kt-container--fluid ">
	<h3 class="kt-subheader-search__title" style="margin-top:10px; margin-bottom:20px">
	    <fmt:message key="app.title.out.slip.list" />
	</h3>
    <!--begin: Search Form -->
	<form class="kt-form kt-form--fit kt-margin-b-20">
										<div class="row kt-margin-b-20">
                                            <div class="col-lg-3">
                                               ...
                                               <input name="datefilter" type="text" class="form-control" id="dateRangeId" readonly="" placeholder="Select time">
                                            </div>
											<div class="col-lg-3 kt-margin-b-10-tablet-and-mobile">
												<label>Status:</label>
                                                <select id="status" class="form-control kt-input" data-col-index="2">
                                                    <option value="0"><fmt:message key="option_all" /></option>
                                                    <option value="1"><fmt:message key="status.out.slip.generated" /></option>
                                                    <option value="2"><fmt:message key="status.out.slip.validated" /></option>
                                                     <option value="3"><fmt:message key="status.out.slip.delivered" /></option>
												</select>
											</div>
											<div class="col-lg-3 kt-margin-b-10-tablet-and-mobile">
...
<%--                                                     <c:forEach var="office" items="${userOffice}"> --%>
...
<%--                                                     </c:forEach> --%>
                                            ...
                                            <div class="col-lg-3 kt-margin-b-10-tablet-and-mobile">
                                               <center>
                                                 <button style="width: 50%; margin-top:20px;" type="button" class="btn btn-success" title="search In-Slip" onClick="searchButtonOutSlipList();">
										...
										<div class="row kt-margin-b-20">
                                            <div class="col-lg-3 kt-margin-b-10-tablet-and-mobile">
												...
												<input id="referenceNum" type="text" class="form-control kt-input" placeholder="DTR-5-O1188" data-col-index="0">
											</div>
    		...
    		<div class="kt-portlet__body">
    		 ...
    		 <table class="table table-striped- table-bordered table-hover table-checkable dataTable dtr-inline" id="kt_table_1" role="grid" aria-describedby="kt_table_1_info" style="position: relative; width: 1644px;">
				<thead style="background:#01A8F9;">
					...
					<th class="sorting" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 152.25px;" aria-label="Ship City: activate to sort column ascending"><fmt:message key="app.in.slip.delivery.date" /></th>
					...
					<th class="sorting" tabindex="0" aria-controls="kt_table_1" rowspan="1" colspan="1" style="width: 73.25px;" aria-label="Ship Date: activate to sort column ascending">ACTIONS</th>
				        ...
				        <c:if test="${user.group.id == 2 }">
				            <c:set var = "i"  value = "1"/>
    						<c:forEach var="outSlip" items="${outSlipsByOffice}">
    						    <c:if test ="${i%2==1}">
                                    <tr  role="row" class="odd clear-odd">
                                        <td tabindex="0" class="sorting_1"><c:out value = "${i}"/></td>
                                        <td>${outSlip.referenceNumber}</td>
                                        <fmt:formatDate value="${outSlip.creationDate}" pattern="dd/MM/yyyy hh.mm,s" var="creationDate" />
                                        <td>${creationDate}</td>
                                        <fmt:formatDate value="${outSlip.deliveryDate}" pattern="dd/MM/yyyy hh.mm,s" var="deliveryDate" />
                                        <td>${deliveryDate}</td>

                                        <!-- start managing status -->
                                        <c:if test="${outSlip.status == 1 }">
                                        <td id='outSlip-status-${outSlip.id}' ><fmt:message key="outSlip.status.generated" /></td>
                                        </c:if>
                                        <c:if test="${outSlip.status == 2 }">
                                        <td id='outSlip-status-${outSlip.id}' ><fmt:message key="outSlip.status.validated" /></td>
                                        </c:if>
                                        <c:if test="${outSlip.status == 3 }">
                                        <td id='outSlip-status-${outSlip.id}' ><fmt:message key="outSlip.status.delivered" /></td>
                                        </c:if>
                                        <c:if test="${outSlip.status == 4 }">
                                        <td id='outSlip-status-${outSlip.id}' ><fmt:message key="outSlip.status.confirmed" /></td>
                                        </c:if>
                                        <!-- end  managing status -->

                                        <td>${outSlip.total}</td>
                                        <td>${outSlip.office.name}</td>
                                        <td >
                                            <select  onchange="outSlip_action(this.value,'${outSlip.id}','outSlip');return false;" class="form-control input-sm">
                        ...
                        <c:if test="${user.group.id != 2 }">
				            <c:set var = "i"  value = "1"/>
    						<c:forEach var="outSlip" items="${outSlips}">
    						    <c:if test ="${i%2==1}">
                                    <tr  role="row" class="odd clear-odd">
                                        <td tabindex="0" class="sorting_1"><c:out value = "${i}"/></td>
                                        <td>${outSlip.referenceNumber}</td>
