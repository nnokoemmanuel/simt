			...
			<div class="kt-portlet">
				...
				<div  id="load-preview-step1" style="color:white;margin-left:42%;margin-right:42%;" > </div>
				    ...
				    <form class="kt-form kt-form--label-right" id="add_applicant_form" name="add_applicant_form">
					    <input id="applicantId" type="hidden" value="${applicantForm.id}">
					    <input id="entranceEligibleCenterId" name="entranceEligibleCenterId" type="hidden" value="${entranceEligibleCenterId}">
							...
							<div id="divBlock">
								...
								<div class="col-lg-4">
									...
									<select name="speciality"  id="speciality" class="form-control">
										...
										<c:forEach var="eligibleSpeciality" items="${eligibleSpecialities}">
											<c:if test = "${eligibleSpeciality.id==2}">
												<option value="${eligibleSpeciality.id }">${eligibleSpeciality.abbreviation}</option>
											</c:if>
											<c:if test = "${eligibleSpeciality.id==3}">
												<option value="${eligibleSpeciality.id }">${eligibleSpeciality.abbreviation}</option>
											</c:if>
		                                </c:forEach>
									</select>
						...
						<div id="divBlock2" style="display:none;">
                            ...
                            <div class="form-group row" id="div4">
                                <div class="col-lg-4">
									<label style="color:#564FC1;"><fmt:message key="training.center.first.choice" />*</label>
									<select name="trainingCenterChoice1"  id="trainingCenterChoice1" class="form-control">
										<option value=""><fmt:message key="select.fill.training.center"/></option>
										<c:forEach var="trainingCenter" items="${allTrainingCenters}">
		                                	<option value="${trainingCenter.id }">${trainingCenter.name}</option>
		                                </c:forEach>
									</select>
								</div>
                                <div class="col-lg-4">
									<label style="color:#564FC1;"><fmt:message key="training.center.second.choice" /></label>
									<select name="trainingCenterChoice2"  id="trainingCenterChoice2" class="form-control">
										<option value=""><fmt:message key="select.fill.training.center"/></option>
                                        <c:forEach var="trainingCenter" items="${allTrainingCenters}">
		                                	<option value="${trainingCenter.id }">${trainingCenter.name}</option>
		                                </c:forEach>
                                ...
                                <div class="col-lg-4">
									<label style="color:#564FC1;"><fmt:message key="training.center.third.choice" /></label>
									<select name="trainingCenterChoice3"  id="trainingCenterChoice3" class="form-control">
										<option value=""><fmt:message key="select.fill.training.center"/></option>
                                        <c:forEach var="trainingCenter" items="${allTrainingCenters}">
		                                	<option value="${trainingCenter.id }">${trainingCenter.name}</option>
		                                </c:forEach>
                                    ...
                                    <div class="kt-form__actions">
                                        <div class="row">
                                            <div class="col-lg-5"></div>
                                                <div class="col-lg-7">
                                                    <button type="button" class="btn btn-brand"  id="create_applicant" onclick="registerApplicant();" >Submit</button>
